...
import pytest
import sys
from kirkapy.cli import main

@pytest.fixture
def mock_client():
    with patch("kirkapy.KirkaClient", autospec=True) as m:
...
def test_cli_user(mock_client):
    ...
    with patch.dict("os.environ", {"KIRKA_API_KEY": "test"}):
        with patch.object(sys, "argv", ["kirkapy", "user", "S1"]):
            main()
...
def test_cli_clan(mock_client):
    ...
    with patch.dict("os.environ", {"KIRKA_API_KEY": "test"}):
        with patch.object(sys, "argv", ["kirkapy", "clan", "C1"]):
            main()
...
def test_cli_leaderboard(mock_client):
    ...
    with patch.dict("os.environ", {"KIRKA_API_KEY": "test"}):
        with patch.object(sys, "argv", ["kirkapy", "leaderboard", "solo"]):
            main()
